...
colnames = ['Date', "Site", "Item"] + list(map(str, list(range(24))))
...
# melt "Hour" to column
df = pd.melt(df, id_vars=['Date', 'Item'], value_vars=list(map(str, list(range(24)))), var_name="Hour",
             value_name="Value")

# generate "Datetime"
df["Datetime"] = pd.to_datetime(df["Date"] + " " + df["Hour"] + ":00:00")
df = df[['Datetime', "Item", "Value"]]
